output "alb_sg_id" {
  ...
  value       = element(concat(
                  aws_security_group.allow_in80_in443_outALL.*.id, tolist([var.alb_custom_security_group_id])), 0)
}

output "alb_sg_ingress" {
  description = "The Ingress rule of security group for ALB."
  value       = element(concat(aws_security_group.allow_in80_in443_outALL.*.id, tolist(["Please refer to ${var.alb_custom_security_group_id}"])), 0)
}
...
output "alb_s3_logs" {
  ...
  value       = element(concat(aws_s3_bucket.alb-logs.*.id, tolist([""])), 0)
}
...
output "alb_custom_dns_name" {
  ...
  value       = element(concat(aws_route53_record.alb.*.name, data.aws_iam_server_certificate.ss_cert.*.name, tolist([""])), 0)
}
...
output "root_domain_hosted_zone_id" {
  value = element(concat(aws_route53_record.alb.*.zone_id, tolist([""])), 0)
}
